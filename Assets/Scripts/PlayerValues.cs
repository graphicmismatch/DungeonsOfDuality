...
public class PlayerValues : MonoBehaviour
{
    ...
    void Update()
    {
        if (this.transform.position == new Vector3(0, 0, -4))
        {
            SceneChan.change("ZeroEnd");
        }
        if (health <= 0)
        {
            dead = true;
        ...
        if (!dead)
        {
            ...
            if (Input.GetMouseButtonDown(0) && !monstermode && PlayerMovement.inst.checkinput)
            {
    ...
    public static void Hurt(float x)
    {
        if (x > 0)
        {
            AudioSource.PlayClipAtPoint(inst.hurts, inst.transform.position);
        }
        if (x < 0)
        {
            AudioSource.PlayClipAtPoint(inst.heal, inst.transform.position);
        }
        health -= x;
        if (health <= 0)
        {
